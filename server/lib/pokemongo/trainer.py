...
class Trainer(object):
    ...
    def encounterAndCatch(self, pokemon, thresholdP=0.5, limit=5, delay=2):
        ...
        while True:
            ...
            # Success or run away
            if attempt.status == 1:
                self.store_in_db(pokemon, catch_status=attempt.status, with_ball=bestBall)
                return attempt
            ...
            if count >= limit:
                ...
                self.store_in_db(pokemon, catch_status=100 + count)
                return None

    # Walk over to position in meters
    def walkTo(self, olatitude, olongitude, epsilon=10, step=7.5, search_each=None,
               delay=10, walk_and_catch_callback=None):
        if step >= epsilon:
        ...
        while dist > epsilon:
            ...
            # Search each x seconds for pokemon
            if search_each:
                if walk_and_catch_callback and seconds > search_each:
